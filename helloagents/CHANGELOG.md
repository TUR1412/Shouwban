...
## [20260118.2] - 2026-01-18

### 修复
- UI：首页精选/策展商品在 dist（GitHub Pages）环境下因 `ProductListing` 全局缺失导致的报错与空白（增加全局兼容层，首页/详情页统一复用）
- PLP：高级筛选空输入被错误解析为 `0`，触发“多级筛选”并将商品列表过滤为 0 的问题
- UI：Header 动作区在小屏出现横向溢出导致布局挤压/重叠的问题

### 变更
- 版本：统一缓存穿透版本号 bump 到 `20260118.2`

## [20260118.1] - 2026-01-18

### 修复
- UI：首页 `index.html` 清理多余 `</div>`，修复 Header/Hero/策展区块在部分解析路径下的布局重叠/塌陷
- UI：Header 移动端菜单开合与搜索栏/下拉互斥，避免多浮层叠加遮挡

### 变更
- 版本：统一缓存穿透版本号 bump 到 `20260118.1`
